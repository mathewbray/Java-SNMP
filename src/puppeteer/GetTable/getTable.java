  ...
  public Vector getTablev1(String ip, String puerto, String comunidad, int intentos, int tiempoEspera, int[] requerimiento,long limite){
  	...
  	while(!termino){
  	  ...
  	  if(resultado.indexOf("Error")!=(-1)){
  	  	...
  	  	interno = new Vector();
  	  	//se aade el resultado al vector para indicar la columna
		...
		while(Token.hasMoreTokens()){
		  //Se aaden los datos de la columna al vector columna
		  String dato = String.valueOf(Token.nextToken());
		  //System.out.println("se va a aadir un: "+dato);
		  interno.addElement(dato);
		}
		//System.out.println(interno.size());
		//se aaden las columnas al vector general
  	  	respuesta.addElement(interno);
  	  	//se aade el OID de la columna al vector nombre de columnas
	...
	//System.out.println("El tamao de l arespuesta es de: "+respuesta.size());
  	return respuesta;
  }
